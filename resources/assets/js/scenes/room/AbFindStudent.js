  ...
  render() {
    ...
    return (
      <div className='action-bar action-bar-find-student' onKeyDown={e => this.handleKeyDown(e)}>
        ...
        <CSSTransition
          in={this.state.showSearchInput}
          timeout={300}
          classNames='input-container'
        >
          <div className="input-container">
            <i className="far fa-search"></i>
            <input type='text' ref={this.filterRef} placeholder="Type to find student..." onChange={(e) => this.handleSearchInput(e)} value={this.state.query}/>
          </div>
        </CSSTransition>
          ...
          <ul>{ filteredStudents.map(student =>
            ...
            { filteredStudents.length == 0
              ? unseatedStudents.length == 0
                ...
                : <li className='no-results'>No unseated students found with "{this.state.query}"</li>
              : false
            }
          </ul>
        </div>
