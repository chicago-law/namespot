...
export function saveNewTable(tableID, roomId, coords, seatCount, labelPosition) {
  return (dispatch, getState) => {
    ...
    let formattedCoords = {
      sX: null, sY: null, eX: null, eY: null, qX: null, qY: null,
    }
    Object.keys(coords).forEach((coordType) => {
      const coord = coords[coordType]
      if (coord && coord !== 'null_null') {
        const split = coord.split('_')
        switch (coordType) {
          case 'start':
            formattedCoords = {
